...
function EnderecoHome() {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await axios.get('http://localhost:8000/endereco/');
  ...
  useEffect(() => {
    ...
    cliente.rua && cliente.rua.toLowerCase().includes(searchNome.toLowerCase())
    );
  ...
  const handleDelete = async (id) => {
    if (!isNaN(id)) {
      try {
        const response = await axios.delete(`http://localhost:8000/endereco/${id}`);
